<template>
    <div class="flex flex-col items-center gap-4 full-page justify-center">   
        ...
        <textarea v-model="descripcion" :disabled="isDisabled" :placeholder="isDisabled?'No encontramos la tarea':'Escribe tu tarea'" tile="Escribe tu tarea" class="disabled:border-gray-200 h-40 w-96 border-2 border-solid rounded-xl p-2 focus-within:outline-2 focus-within:outline-gray-600"></textarea>
        <template v-if="!isDisabled">
            <button @click="editarTarea()" class="bg-black rounded-3xl px-4 py-2 text-white hover:bg-gray-700 ">Guardar tarea</button>
        </template>
        <template v-else>
            <NuxtLink to="/tareas" class="bg-black rounded-3xl px-4 py-2 text-white hover:bg-gray-700 ">Volver</NuxtLink>
        </template>
